...
import { CheckIcon, Group, ICONS, IconImage } from './settingsShared';
    ...
    <div class={styles.content}>
      ...
      <Group>
        <For each={APP_DEFINITIONS.filter((app) => props.phoneState.enabledApps.includes(app.id))}>
          {(app) => {
            const unreadCount = props.notificationsActions.getUnreadCount(app.id);
            ...
            return (
              <div class={styles.appRow}>
                ...
                <div class={styles.appInfo}>
                  ...
                  <div class={styles.appStatus}>{muted ? 'Notificaciones desactivadas' : unreadCount > 0 ? t('settings.unread_count', props.language(), { count: unreadCount }) : t('settings.up_to_date', props.language())}</div>
                </div>

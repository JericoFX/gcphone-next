...
interface IslandMusicProps {
  activity: LiveActivity;
}

export function IslandMusic(props: IslandMusicProps) {
  return (
    <div class={styles.expandedContent}>
      <div class={styles.expandedHeader}>
        <span class={`${styles.dot} ${styles.dotMusic}`} />
        <div style={{ flex: '1', 'min-width': '0' }}>
          <div class={styles.expandedTitle}>{props.activity.title}</div>
          <div class={styles.expandedSubtitle}>{props.activity.subtitle || 'Reproduciendo'}</div>
        </div>
      </div>

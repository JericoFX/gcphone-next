...
lib.callback.register('gcphone:yellowpages:getListings', function(source, data)
    if Utils.HitRateLimit(source, 'yp_listings', 1500, 4) then return {} end
...
lib.callback.register('gcphone:yellowpages:createListing', function(source, data)
    ...
    if Utils.HitRateLimit(source, 'yp_create', 3000, 1) then return false, 'RATE_LIMITED' end
    local identifier = Bridge.GetIdentifier(source)
    ...
    local playerName = Bridge.GetName(source) or 'Usuario'
...
lib.callback.register('gcphone:yellowpages:deleteListing', function(source, listingId)
    ...
    if Utils.HitRateLimit(source, 'yp_delete', 1500, 2) then return false end
    local identifier = Bridge.GetIdentifier(source)
...
lib.callback.register('gcphone:yellowpages:shareLocation', function(source, listingId)
    if Phone.IsPhoneReadOnly(source) then return false end
    if Utils.HitRateLimit(source, 'yp_location', 2000, 2) then return false end
    local identifier = Bridge.GetIdentifier(source)
    if not identifier then return false end

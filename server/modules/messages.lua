...
lib.callback.register('gcphone:wavechatCreateGroup', function(source, data)
    ...
    local memberIdentifiers = { [identifier] = true }
    ...
    for _, entry in ipairs(members) do
        ...
        if number ~= '' then
            ...
            if targetIdentifier and not memberIdentifiers[targetIdentifier] then
    ...
    for memberIdentifier, _ in pairs(memberIdentifiers) do
        local role = memberIdentifier == identifier and 'owner' or 'member'
        MySQL.insert.await(
            'INSERT INTO phone_chat_group_members (group_id, identifier, role) VALUES (?, ?, ?)',
            { groupId, memberIdentifier, role }
        )
    end

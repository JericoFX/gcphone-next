...
local function GetChannel(company, phoneNumber)
    local channel = MySQL.scalar.await("SELECT id FROM phone_services_channels WHERE company = ? AND phone_number = ?",
        { company, phoneNumber })
    ...
    return MySQL.insert.await("INSERT INTO phone_services_channels (company, phone_number) VALUES (?, ?)",
        { company, phoneNumber })
end
...
BaseCallback("services:sendMessage", function(source, phoneNumber, channelId, company, message, anonymous)
    ...
    MySQL.update("UPDATE phone_services_channels SET last_message = SUBSTRING(?, 1, 50) WHERE id = ?",
        { message, channelId })
...
BaseCallback("services:deleteChannel", function(source, phoneNumber, channelId)
    ...
    local success = MySQL.update.await("DELETE FROM phone_services_channels WHERE id = ? AND company = ?",
        { channelId, GetJob(source) }) > 0

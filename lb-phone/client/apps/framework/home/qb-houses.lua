...
local function FormatKeyHolders(keyholders)
    ...
    for i = 1, #keyholders do
        formatted[i] = {
            ...
            name = keyholders[i].charinfo and
            (keyholders[i].charinfo.firstname .. " " .. keyholders[i].charinfo.lastname) or keyholders[i].name
        }
    end
...
RegisterNUICallback("Home", function(data, cb)
    ...
    elseif action == "removeKeyholder" then
        ...
        TriggerServerEvent("qb-houses:server:removeHouseKey", houseData.uniqueId,
            { citizenid = data.identifier, firstname = "", lastname = "" })
        Wait(500)
        QB.Functions.TriggerCallback("qb-phone:server:GetPlayerHouses", function(houses)
            if not houses then
                return cb({})

...
RegisterNUICallback('snapFollow', function(data, cb)
    lib.callback('gcphone:snap:follow', false, function(result)
        cb(result or { following = false })
    end, data or {})
end)

RegisterNUICallback('snapCheckFollowing', function(data, cb)
    local targetAccountId = tonumber(type(data) == 'table' and data.targetAccountId or data)
    lib.callback('gcphone:snap:checkFollowing', false, function(isFollowing)
        cb({ isFollowing = isFollowing == true })
    end, targetAccountId)
end)

RegisterNUICallback('snapGetFollowingFeed', function(data, cb)
    lib.callback('gcphone:snap:getFollowingFeed', false, function(feed)
        cb(feed or {})
    end, data or {})
end)

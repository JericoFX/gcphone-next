...
BEGIN
    IF NEW.recipient_account_id IS NOT NULL THEN
        INSERT INTO `phone_mail_boxes` (`account_id`, `unread_count`, `total_count`)
        VALUES (NEW.recipient_account_id, IF(NEW.is_read = 1, 0, 1), 1)
        ON DUPLICATE KEY UPDATE
...
FOR EACH ROW
BEGIN
    IF OLD.recipient_account_id IS NOT NULL THEN
        UPDATE `phone_mail_boxes`
